...
# pyre-fixme[9]: tok_mask has type `Tensor`; used as `None`.
...
class DividedAttention(nn.Module):
    ...
    def forward(
        ...
        **einops_dims,
        ...
        (cls_q, q_), (cls_k, k_), (cls_v, v_) = [
            (t[:, 0:1], t[:, 1:]) for t in (q, k, v)
        ]
        ...
        q_, k_, v_ = [
            rearrange(t, f"{einops_from} -> {einops_to}", **einops_dims)
            for t in (q_, k_, v_)
        ]

        # expand CLS token keys and values across time or space and concat
        r = q_.shape[0] // cls_k.shape[0]
        cls_k, cls_v = [repeat(t, "b () d -> (b r) () d", r=r) for t in (cls_k, cls_v)]
        ...
        if tok_mask is not None:
            ...
            #  tuple[Tensor, ...]]` but got `Tuple[Optional[Tensor],
            #  Optional[Tensor]]`.
            mask_ = torch.cat((cls_mask, mask_), dim=1)

        # attention
        # pyre-fixme[6]: For 4th argument expected `Tensor` but got `Optional[Tensor]`.
