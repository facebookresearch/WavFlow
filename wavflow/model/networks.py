...
class WavflowModel(nn.Module):
    def __init__(
        ...
        *,
        ...
        # latent_mean/latent_std no longer needed - using audio_tokens directly
        # normalization is handled via audio_scale in data loading
        ...
        self.empty_clip_feat = nn.Parameter(
            torch.zeros(1, clip_dim), requires_grad=True
        )
        self.empty_sync_feat = nn.Parameter(
            torch.zeros(1, sync_dim), requires_grad=True
        )
    ...
    def update_seq_lengths(
        self, latent_seq_len: int, clip_seq_len: int, sync_seq_len: int
    ) -> None:
        self._latent_seq_len = latent_seq_len
        self._clip_seq_len = clip_seq_len
        self._sync_seq_len = sync_seq_len
    ...
    def normalize(self, x: torch.Tensor) -> torch.Tensor:
        # no-op: normalization handled via audio_scale in data loading
        return x

    def unnormalize(self, x: torch.Tensor) -> torch.Tensor:
        # no-op: de-normalization handled via audio_scale when saving
        return x
    ...
    def preprocess_conditions(
        ...
        """
        assert clip_f.shape[1] == self._clip_seq_len, (
            f"{clip_f.shape=} {self._clip_seq_len=}"
        )
        assert sync_f.shape[1] == self._sync_seq_len, (
            f"{sync_f.shape=} {self._sync_seq_len=}"
        )
        assert text_f.shape[1] == self._text_seq_len, (
            f"{text_f.shape=} {self._text_seq_len=}"
        )
    ...
    def predict_flow(
        ...
        """
        assert latent.shape[1] == self._latent_seq_len, (
            f"{latent.shape=} {self._latent_seq_len=}"
        )
        ...
        latent = self.audio_input_proj(latent)  # (B, N, D) (B, 250, D)

        global_c = self.global_cond_mlp(clip_f_c + text_f_c)  # (B, D)

        global_c = self.t_embed(t).unsqueeze(1) + global_c.unsqueeze(1)  # (B, 1, D)
        extended_c = global_c + sync_f  # (B, 192, 448)

        for block in self.joint_blocks:
            if self.gradient_checkpointing and self.training:
                latent, clip_f, text_f = gradient_checkpoint(
                    block,
                    latent,
                    clip_f,
                    text_f,
    ...
    def get_empty_conditions(
        self, bs: int, *, negative_text_features: Optional[torch.Tensor] = None
    ) -> PreprocessedConditions:
...
def medium_16k(latent_seq_len: int = 640, **kwargs) -> WavflowModel:
    num_heads = 14
    return WavflowModel(
        latent_dim=200,
        clip_dim=1024,
...
def medium_44k(latent_seq_len: int = 1764, **kwargs) -> WavflowModel:
    num_heads = 14
    return WavflowModel(
        latent_dim=200,
        clip_dim=1024,
...
def large_16k(latent_seq_len: int = 640, **kwargs) -> WavflowModel:
    num_heads = 14
    return WavflowModel(
        latent_dim=200,
        clip_dim=1024,
...
def large_44k(latent_seq_len: int = 1764, **kwargs) -> WavflowModel:
    num_heads = 14
    return WavflowModel(
        latent_dim=200,
        clip_dim=1024,
...
_MODEL_REGISTRY: dict[str, Callable[..., WavflowModel]] = {
    "medium_16k": medium_16k,
...
def get_wavflow_model(name: str, **kwargs) -> WavflowModel:
...
if __name__ == "__main__":
    network = get_wavflow_model("medium_16k")
